...
def make_test_dict(pac, msg_bits, snr_range):

    test_dict = {}
    test_dict['msg'] = msg_bits
    pac_code = pac.pac_encode(msg_bits, scheme = args.rate_profile)

    test_dict['rec'] = {}
    for snr in snr_range:
        noisy_code = pac.channel(pac_code, snr, args.noise_type, args.vv, args.radar_power, args.radar_prob)
        test_dict['rec'][snr] = noisy_code
...
class RNN_inference(RNN_Model): # for snpe
    def __init__(self, decoding_type, N, info_inds, onehot, rnn_type, input_size, feature_size, output_size, num_rnn_layers, y_size, y_hidden_size, y_depth, activation = 'relu', dropout = 0., skip=False, out_linear_depth=1, out_linear_size = None ):
        super().__init__(rnn_type, input_size, feature_size, output_size, num_rnn_layers, y_size, y_hidden_size, y_depth, activation, dropout, skip, out_linear_depth, out_linear_size)
        self.N = N
        self.decoding_type = decoding_type
        self.info_inds = info_inds
        self.onehot = onehot

    def f_theta(self, input, hidden, Fy=None):
    ...
    def forward(self, y):
        ...
        decoded = torch.ones(y.shape[0], self.N, device = y.device)
        if self.decoding_type == 'y_h0':
            hidden = self.get_h0(y)
            for ii in range(0, args.N): # don't assume first bit is always frozen
                if ii == 0:
                    out, hidden = self.f_theta(onehot_fn(torch.ones(y.shape[0], device = y.device)).view(-1, 1, self.input_size),  hidden)
                else:
                    out, hidden = self.f_theta(onehot_fn(decoded[:, ii-1].sign()).view(-1, 1, self.input_size), hidden)
                if ii in self.info_inds:
                    decoded[:, ii] = out.squeeze().sign()
        elif self.decoding_type == 'y_input':
            hidden = torch.zeros(self.num_rnn_layers, y.shape[0], self.feature_size, device = y.device)
            if self.rnn_type == 'LSTM':
                hidden = (hidden, hidden)
            for ii in range(0, args.N): # don't assume first bit is always frozen
                if ii == 0:
                    out, hidden = self.f_theta(torch.cat([y.unsqueeze(1), onehot_fn(torch.ones(y.shape[0], device = y.device)).view(-1, 1, self.input_size - args.N)], 2), hidden)
                else:
                    out, hidden = self.f_theta(torch.cat([y.unsqueeze(1), onehot_fn(decoded[:, ii-1].sign()).view(-1, 1, self.input_size - args.N).detach().clone()], 2), hidden)
                if ii in self.info_inds:
                    decoded[:, ii] = out.squeeze().sign()
        elif self.decoding_type == 'y_h0_out':
            hidden = self.get_h0(y)
            Fy = hidden.clone().permute(1, 0, 2).contiguous().reshape(-1, 1, self.num_rnn_layers*self.feature_size)
            for ii in range(0, args.N): # don't assume first bit is always frozen
                if ii == 0:
                    out, hidden = self.f_theta(onehot_fn(torch.ones(y.shape[0], device = y.device)).view(-1, 1, self.input_size),  hidden, Fy)
                else:
                    out, hidden = self.f_theta(onehot_fn(decoded[:, ii-1].sign()).view(-1, 1, self.input_size), hidden, Fy)
                if ii in self.info_inds:
                    decoded[:, ii] = out.squeeze().sign()
        return decoded[:, self.info_inds]
