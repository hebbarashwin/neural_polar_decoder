...
class smallNet(nn.Module):
    def __init__(self,config):
        ...
        self.layers = nn.Sequential(
            nn.Linear(self.output_len , self.hidden_dim),
            nn.GELU(),
...
class multiplyFeature(nn.Module):
    def __init__(self,mat):
        super(multiplyFeature,self).__init__()
        self.register_buffer('mat',mat)
    def forward(self,x,device):
        matOut = x.unsqueeze(1)*self.mat
        matOut = matOut.masked_fill((matOut==0),1.)
        return torch.prod(matOut,-1)
        
class multiplyLayer(nn.Module):
    def __init__(self,dim):
        super(multiplyLayer,self).__init__()
        self.linearOut = nn.Linear(dim,dim)
    def forward(self,x1,x2):
        prod = x1*x2
        linOut = self.linearOut(prod)
        return linOut

class convUnit(nn.Module):
    def __init__(self,config):
        super(convUnit,self).__init__()
        self.hidden_dim = int(config.embed_dim/8)
        ...
        self.layersFin = nn.Sequential(
            nn.Linear(self.input_len*self.hidden_dim , 1),
            )
    ...
    def forward(self,noisy_enc):
        ...
        output = self.layer_norm(self.dropout(self.layersFin(torch.flatten(input6,start_dim=1)))).squeeze()
        
        return output 

class bitConvNet(nn.Module):
    def __init__(self,config):
        super(bitConvNet,self).__init__()
        self.hidden_dim = int(config.embed_dim/2)
        self.input_len = config.max_len
        self.output_len = config.N
        self.nets = nn.ModuleList([convUnit(config) for _ in range(self.output_len)])
        
        
    def forward(self,noisy_enc,mask,trg_seq,device):
        output = torch.ones((noisy_enc.shape[0],self.output_len))
        for i in range(self.output_len):
            output[:,i] = self.nets[i](noisy_enc)
        ...
        output = torch.cat((1-output,output),-1)
        out_mask = mask
        return output,decoded_msg_bits,out_mask,logits,input4
...
class multConvNet(nn.Module):
    def __init__(self,config,mat):
        super(multConvNet,self).__init__()
        self.hidden_dim = int(config.embed_dim/2)
        self.input_len = config.max_len
        self.output_len = config.N
        self.mat = mat
        ...
        self.feature1 = multiplyFeature(mat)
        self.layers1 = nn.Sequential(
            nn.Conv1d(1,int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
        ...
        #self.mult1 = multiplyLayer(config.N)
        #self.layer_norm1 = nn.LayerNorm(self.output_len, eps=1e-6)
        self.layers2 = nn.Sequential(
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
        ...
        #self.mult2 = multiplyLayer(config.N)
        #self.layer_norm2 = nn.LayerNorm(self.output_len, eps=1e-6)
        self.layers3 = nn.Sequential(
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
        ...
        #self.mult3 = multiplyLayer(config.N)
        #self.layer_norm3 = nn.LayerNorm(self.output_len, eps=1e-6)
        self.layers4 = nn.Sequential(
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim/2),self.kernel,padding=self.padding,bias=bias),
        ...
        #self.mult4 = multiplyLayer(config.N)
        #self.layer_norm4 = nn.LayerNorm(self.output_len, eps=1e-6)
        self.layers5 = nn.Sequential(
            nn.Conv1d(int(self.hidden_dim/2),int(self.hidden_dim),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(self.hidden_dim,self.hidden_dim,self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            )
        #self.mult5 = multiplyLayer(config.N)
        #self.layer_norm5 = nn.LayerNorm(self.output_len, eps=1e-6)
        self.layersFin = nn.Sequential(
    ...
    def forward(self,noisy_enc,mask,trg_seq,device):
        ...
        #inputFeat = self.feature1(input1,device)
        input2 = self.layers1(input1) #+ inputFeat.unsqueeze(1)
        #input2 = self.layer_norm1(self.mult1(input2[:,:int(input2.shape[1]/2)],input2[:,int(input2.shape[1]/2):]))
        
        residual2 = input2
        input3 = self.layers2(input2) + residual2
        #input3 = self.layer_norm2(self.mult2(input3[:,:int(input3.shape[1]/2)],input3[:,int(input3.shape[1]/2):])) + residual2
        
        residual3 = input3
        input4 =  self.layers3(input3) + residual3
        #input4 = self.layer_norm3(self.mult3(input4[:,:int(input4.shape[1]/2)],input4[:,int(input4.shape[1]/2):])) + residual3
        
        residual4 = input4
        input5 =  self.layers4(input4) + residual4
        #input5 = self.layer_norm4(self.mult4(input5[:,:int(input5.shape[1]/2)],input5[:,int(input5.shape[1]/2):])) + residual4
...
class simpleNet(nn.Module):
    def __init__(self,config):
        ...
        self.layers1 = nn.Sequential(
            nn.Conv1d(1,self.hidden_dim,self.kernel,padding=self.padding),
            nn.GELU(),
            nn.Conv1d(self.hidden_dim,self.hidden_dim,self.kernel,padding=2*self.padding,dilation=2),
            nn.GELU(),
            nn.Conv1d(self.hidden_dim,self.hidden_dim,self.kernel,padding=self.padding),
            nn.GELU(),
            )
        self.layers2 = nn.Sequential(
            nn.Conv1d(self.hidden_dim,self.hidden_dim,self.kernel,padding=self.padding),
        ...
        #self.layers3 = nn.Conv1d(self.hidden_dim,1,self.kernel,padding=self.padding)
        self.layers3 = nn.Sequential(
            nn.Linear(self.hidden_dim*self.output_len , self.output_len),
            nn.GELU(),
            nn.Linear(self.output_len , self.output_len),
            nn.GELU(),
            nn.Linear(self.output_len , self.output_len)
    ...
    def forward(self,noisy_enc,mask,trg_seq,device):
        ...
        input2 = self.layers1(input1) + residual1
        residual2 = input2
        output2 = self.layers2(input2) + residual2
        
        #output = self.layers3(output2)
        output = self.layers3(torch.flatten(output2,start_dim=1))
        #print(output.size())
        logits = output.squeeze().unsqueeze(-1)
        decoded_msg_bits = logits.sign()
        output = torch.sigmoid(logits)
...
class bigConvNet(nn.Module):
    def __init__(self,config):
        ...
        self.layers1 = nn.Sequential(
            nn.Conv1d(1,int(self.hidden_dim),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim),int(self.hidden_dim),self.kernel,padding=2*self.padding,dilation=2,bias=bias),
            nn.GELU(),
            nn.MaxPool1d(5,stride=1, padding=2)
            )
        self.layers2 = nn.Sequential(
            nn.Conv1d(int(self.hidden_dim),int(self.hidden_dim),self.kernel,padding=4*self.padding,dilation=4,bias=bias),
            nn.GELU(),
            nn.Conv1d(int(self.hidden_dim),int(self.hidden_dim),self.kernel,padding=self.padding,bias=bias),
            nn.GELU(),
            nn.MaxPool1d(5,stride=1, padding=2)
            )
        # self.layers3 = nn.Sequential(
    ...
    def forward(self,noisy_enc,mask,trg_seq,device):
        ...
        output = (self.layer_norm(self.layersFin(torch.flatten(input3,start_dim=1))))#self.layer_norm(self.dropout(self.layersFin(torch.flatten(input6,start_dim=1))))
        #print(output.size())
        logits = output.squeeze().unsqueeze(-1)
        decoded_msg_bits = logits.sign()
        output = torch.sigmoid(logits)
        output = torch.cat((1-output,output),-1)
        out_mask = mask
        return output,decoded_msg_bits,out_mask,logits,input2 # [b_size,block_len,2]
